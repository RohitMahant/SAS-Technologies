...
export default function Header() {
  ...
  const getNavItemClasses = (page) =>
    `cursor-pointer ${activePage === page ? "font-cocoBold text-red-800" : "hover:text-textColor"}`;
  ...
  return (
    <>
      ...
      <div className="m-4" onClick={toggleSidebar}>
          {isSidebarOpen ? (
            ...
            <IoMenuSharp size={30} color="#03045E" className="cursor-pointer hover:text-textColor" />
          )}
        </div>
        <div>
    ...
    >
      ...
      <ul className="font-cocoRegular text-1xl text-textColor space-y-12 p-6">
        <li className={getNavItemClasses("HOME")} onClick={() => handleNavClick("HOME", "/")}>
          <Link href="/">HOME</Link>
        </li>
        ...
        <li className={getNavItemClasses("CONTACT US")} onClick={() => handleNavClick("CONTACT US", "/contact")}>
